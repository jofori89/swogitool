...
var Deckbuilder = {
	...
	pagecount: 1,
	pagelimit: 36,
	deck: { character: null, cards: [], sortOrder: this.typeSort },
	cards: [],
	...
	options: {
		...
		// in cards/<episode>
		episodes: ['0', '1', '2', '3', 'EX1', '4', '5', '6', 'EX2' , '7', 'M','UE1', '8', 'EX3', '9', '10', 'UE2', '11', '12', '13', 'EX4', '14', '15', '16', 'UE3', 'EX5', '17', 'UE4', '18', 'EX6', '19', 'UE5', '20', 'EX7'], // Array
		defaultEpisodes: ['20'],
	...
	templateCardlistCard: function (card) {
		...
		var factionImg = 'assets/'+card.faction+'.png';
		...
		/*110280, 200467, 300581*/
			...
			cardImg = Deckbuilder.img('http://211.233.57.198/SG_Game/assets/maintenance/cards/'+card.id+'.png');
		}
		...
		return '<tr class="cardlist-card type-'+card.type+'">\
				<td class="card-fullimage">\
					<div class="wrapper-firefox-fix">\
						<div class="wrapper-cardbg">\
							<img class="cardbg" data-cardid="'+card.id+'" alt="Sword Girls game, card ' + card.name + '" src="'+ cardImg +'" />\
						</div>\
					</div>\
				</td>\
				...
				<td class="card-info '+card.type+'">\
					<div class="wrapper-info">\
						<big title="'+(card.type == 'follower' ? card.stats.atk+' / '+card.stats.def+' / '+card.stats.stm : 'Spell')+'" data-cardid="'+card.id+'" class="card-name">'+card.name+'</big>\
					</div>\
				</td>\
				<td class="card-thumb"><img class="card-thumbnail" date-index="'+card.id+'" src="'+ cardImg +'" alt="Sword Girls game, card ' + card.name + '" /></td>\
	...
	templateCardPreview_Follower: function (card) {
		return '<tr><th>Episode</th><td>'+card.episode+'</td></tr>\
			...
			'+( card.artist != '' ? '<tr><th>Artist</th><td><a style="text-decoration: none;" href="http://swogitools-v2.net63.net/?filter=' + card.artist + '&episodes=All" target="_self" >' +card.artist+ '</a></td></tr>' : '')+'\
	...
	templateCollectionCard_Follower: function (card) {
		...
		return '<li data-cardid="'+card.id+'" class="collection-card faction-'+card.faction+'">\
			<img data-cardid="'+card.id+'" class="card-image" src="'+ cardImg +'" width="120" height="180" alt="Sword Girls game, card ' + card.name + '" title="' + card.name + '" />\
			...
			<img class="card-faction" data-cardid="'+card.id+'" src="'+factionImg+'" alt="Sword Girls game, card faction ' + card.faction + '" />\
	...
	templateDeckLoadWindow: function (i, saves) {
		return '<li data-idx="'+i+'"><a class="saves-saveddeck" data-saveid="'+saves[i].id+'">'+saves[i].name+'</a>\
			<img data-id="'+saves[i].id+'" class="saves-edit" src="'+Deckbuilder.img('http://swogitools-v2.net63.net/assets/edit.png')+'" alt="" />\
			<img data-id="'+saves[i].id+'" class="saves-del" src="'+Deckbuilder.img('http://swogitools-v2.net63.net/assets/close.png')+'" alt="" /></li>';
	},
	...
	templateViewDeckCard: function (card, cardnumber) {
		...
		return '<li class="'+(cardnumber > 5 ? 'deck-card' : 'hand-card')+'">\
			<img class="testdeck-card" data-cardidx="'+card.viewdeck_idx+'" src="'+ cardImg +'" width="120" height="180" alt="Sword Girls game, card ' + card.name + '" />\
	 ...
	 */
	img: function (url) {
		if (Deckbuilder.options.datastream == null)
		{
	...
	bootstrap: function (options) {
		...
		$('#collection-controls .controls-episode').html('');
		...
		$('#collection-controls .controls-size').html('');
		...
		$('#collection-controls .controls-rarity').html('');
		...
		$('#collection-controls .controls-faction').html('');
		...
		$('#collection-controls .controls-type').html('');
		...
		var types = allOption.concat(['+GLM -PRS', '+WIS -SNS', '+PRS -GLM', '+SNS -WIS']);
		$('#collection-controls .controls-archetype').html('');
		for (var i = 0; i < types.length; ++i)
		{
		...
		$('#collection_reset').click(function (event) {
		...
		// setup deck sorting
		$('#sorting .sorting-sortbytype').click(function (event) {
			Deckbuilder.deck.sortOrder = Deckbuilder.typeSort;
			Deckbuilder.refreshDeck();
			return false;
		});
		$('#sorting .sorting-sortbysize').click(function (event) {
		...
		$('#DeckViewer').live('click', function (event) {
		...
		$('#testdialog-help').live('click', function (event) {
			Deckbuilder.testdialog_help.dialog('open');
			event.preventDefault();
		});
		$('#testdialog-shuffle').live('click', function (event) {
			Deckbuilder.shuffle(Deckbuilder.viewdeck);
			Deckbuilder.shuffles++;
			Deckbuilder.testdialogRefresh();
			event.preventDefault();
		});
		$('#testdialog-new').live('click', function(event) {
			Deckbuilder.testdialog_round = 1;
			Deckbuilder.shuffles = 0;
			Deckbuilder.testdialogInit();
			event.preventDefault();
		});
		$('#testdialog-reset').live('click', function(event) {
		...
		$('#testdialog-nextround').live('click', function(event) {
			Deckbuilder.testdialog_round++;
			Deckbuilder.testdialogRefresh();
			event.preventDefault();
		});
		$('#testdialog .cover').live('click', function (event) {
		...
		$('#testdialog .controls .inc-size').live('click', function (event) {
			clearSelection();
			var idx = parseInt($(this).attr('data-cardidx'));
			var card = null;
			// get card
		...
		$('#testdialog .controls .dec-size').live('click', function (event) {
			clearSelection();
			var idx = parseInt($(this).attr('data-cardidx'));
			var card = null;
			// get card
		...
		$('#collection-controls .controls-filter').blur(function (event) {
			Deckbuilder.filter = $(this).val();
			Deckbuilder.selectionAltered = true;
			Deckbuilder.refresh();
		});
		// setup selects
		$('#collection-controls select').change(function (event) {
			Deckbuilder.selectionAltered = true;
			Deckbuilder.refresh();
		});
		// setup library functions
		$('#preview #library-add').live('click', function (event) {
			clearSelection();
		...
		$('#preview #library-remove').live('click', function (event) {
			clearSelection();
			var cardid = parseInt($(this).attr('data-cardid'));
			var card = Deckbuilder.copyCardById(cardid);
			if (Deckbuilder.library[cardid] == undefined)
		...
		// load window
		$('#loaddialog-saves .saves-saveddeck').live('click', function (event) {
			clearSelection();
		...
		// popup
		$('#collection-controls .controls-filterhelp').CreateBubblePopup({
			innerHtml: '<div class="popupwindow">\
				<p>Type a keyword to find the card(s) by name, ability, artist, ID.</p>\
				<br />\
				<div>You may also type logical expressions. Example:</div>\
				<div class="popup-expression">= ((Stm + Def) > 8 && Def >= 2) || Atk >= 8</div>\
				<p>Which reads "Any card with the sum of it\'s defense and stamina greater then 8 and a minimum of 2 defense; or otherwise a card with at least 8 attack."</p>\
				<br />\
				<p>Premitted operations:</p>\
				<ul>\
					<li>basic arithmetic <tt>+</tt>, <tt>-</tt>, <tt>/</tt>, <tt>*</tt></li>\
					<li>basic logic operators <tt>==</tt>, <tt>&lt;=</tt>, <tt>&gt;=</tt>, <tt>&lt;</tt>, <tt>&gt;</tt>\
					<li>logical operations && (and), || (or), ! (not)</li>\
					<li>basic (loose) string comparison <tt>=~</tt></li>\
					<li>string literals " (double quote) and \' (single quote).</li>\
					<li>paranthesis, use wisely ;)</li>\
				</ul>\
				<p>Variables:</p>\
				<ul>\
					<li>basic: Atk, Def, Stm, Cp, Name, Size, Id, Faction, Limit</li>\
					<li>contextual: hasAbility, isFlw, isSpell, isChar, isVita, isAcademy, isCrux, isDL, isSG</div>\
				<ul>\
				<br />\
			</div>',
			themeName: 'all-black',
			themePath: 'http://swogitools-v2.net63.net/assets/plugins/jquery.bubblepopup.v2.3.1/Install/jquerybubblepopup-theme'
		});
	...
	defaultSort: function (a, b) {
		...
		var factions = { vita: 0, academy: 1, crux: 2, darklore: 3, sg: 4, empire: 5 };
	...
	updatepreview: function (cardid) {
		...
		/*110280, 200467, 300581*/
			...
			cardImg = Deckbuilder.img('http://211.233.57.198/SG_Game/assets/maintenance/cardL/'+card.id+'L.jpg');
		}

			$('#preview .preview-image').attr('src', cardImg);
			//http://211.233.57.198/SG_Game/assets/maintenance/cardL/200695L.jpg
			...
			cardName = '<a style="font-weight:bold; color:#22F; text-decoration:none;" href="card.php?id='+ card.id +'" target="_blank">' + cardName + '</a>';			
	...
	refreshDeck: function () {
		...
		var cardImg;		
		if(character.id == '110280') {
			cardImg = '110280L.jpg';
		}
		/*110280, 200467, 300581*/
		else if(character.id == '100258') {
			cardImg = '100258L.jpg';
		}
		else if(character.id == '100259') {
			cardImg = '100259L.jpg';
		}
		else if(character.id == '100260') {
			cardImg = '100260L.jpg';
		}
		else if(character.id == '100261') {
			cardImg = '100261L.jpg';
		}
		else {
			cardImg = Deckbuilder.img('http://211.233.57.198/SG_Game/assets/maintenance/cards/'+character.id+'.png');
		}
			$('#character .character-image').attr('src', cardImg);
			$('#character .character-faction').attr('src', Deckbuilder.img('http://swogitools-v2.net63.net/assets/'+character.faction+'.png'));
			$('#character .character-faction').attr('data-cardid', character.id);
			$('#character .character-name').text(character.name);
		...
		$('#cardlist .cardlist-card .card-copies').click(function (event) {
			clearSelection();
		...
		// make unselectable
		$('#cardlist .cardlist-card .card-thumbnail').mousedown(function () { return false; })
		// compute deck spell count
		...
		if (character == null || followers+spells != 30)
		{
			...
			// hide showcase link
			$('#sorting .sorting-showcase').html('<s>Showcase</s>')
			// hide test link
		...
		else // valid deck
		{
			...
			$('#sorting .sorting-showcase').html('<a href="showcase.php?deck='+Deckbuilder.showcaseserial+'" target="_blank">Showcase</a>');
			// display test link
		...
		if (match == null)
		{
			...
			if (match == null)
			{
				...
				if (match == null)
				{
					...
					{
						var token = match[2].trim();
						var first = Deckbuilder.compileEquation(match[1].trim(), refs);
						var second = Deckbuilder.compileEquation(match[3].trim(), refs);
						if (token == '*')
						{
	...
	refresh: function () {
		...
		if (Deckbuilder.init)
		{
			...
			$('#linkswindow .mainsite-link img').attr('src', Deckbuilder.img('http://211.233.57.198/SG_Game/assets/maintenance/cardL/'+random_card.id+'L.jpg'));
			$('#linkswindow .mainsite-link img').attr('alt', random_card.name);
			$('#linkswindow .mainsite-link').attr('href', 'card.php?id=' + random_card.id);
			$('#linkswindow .mainsite-link').attr('title', random_card.name);
			...
			// Page Limit
			$('#collection-controls .controls-pager .pager-pagelimit').focusout(function (event) {
			...
			// cardfaction linking
			$('#cardlist .card-faction img').live('click', function (event) {
				window.open('card.php?id='+$(this).attr('data-cardid'));
			});
			$('#preview .preview-characterfaction').live('click', function (event) {
				window.open('card.php?id='+$(this).attr('data-cardid'));
			});
			$('#collection .card-faction').live('click', function (event) {
				window.open('card.php?id='+$(this).attr('data-cardid'));
			});
			$('#character .character-faction').live('click', function (event) {
				window.open('card.php?id='+$(this).attr('data-cardid'));
			});
			$('#character .character-name').live('click', function (event) {
				Deckbuilder.updatepreview($(this).attr('data-cardid'));
			});
			// options binding
			$('#options .options-clear').click(function () {
				Deckbuilder.deck.character = null;
				Deckbuilder.deck.cards = [];
			...
			$('#options .options-save').click(function () {
			...
			$('#options .options-load').click(function () {
					Deckbuilder.loaddialog.dialog('open');
					// populate
					$('#loaddialog-saves').html('');
			...
			$('#loaddialog-saves .saves-del').live('click', function (event) {
			...
			$('#loaddialog-saves .saves-edit').live('click', function (event) {
					var id = $(this).attr('data-id');
					for (var i = 0; i < Deckbuilder.saves.length; ++i)
					{
						if (Deckbuilder.saves[i].id == id)
			...
			if (deckparam != '')
			{
				...
				serial = deckparam;
				deckcompression = getParameterByName('compression');
				if (deckcompression != '')
				{
		...
		if (Deckbuilder.selectionAltered) {
			...
			$('#collection_selection').attr('href', Deckbuilder.options.page_baseurl+'?filter='+filter+'&rarity='+rarity+'&episodes='+episodes+'&sizes='+sizes+'&types='+types+'&factions='+factions+'&archetypes='+archetypes)

			// setup preview; if void
			...
			// make unselectable
			$('#collection .collection-card .card-image').mousedown(function () { return false; })
